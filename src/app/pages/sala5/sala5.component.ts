...
import { SignalRService } from 'src/app/services/signalr.service';
  ...
  export class Sala5Component implements OnInit, AfterContentInit, OnDestroy {
    ...
    ngOnInit(): void {
      ...
      // this.speak();
      this.mensajes = StorageHelper.obtenerMensajes5() || [];  // TODO: HACER UNA FUNCION PARA GUARDAR LOS MENSAJES EN EL LOCAL STORAGE
        ...
        this.signalrService.messageSubscription.subscribe({
          next: (message: IMessage) => {
            ...
            if (message.room === this.currentUser.room) {
              ...
              StorageHelper.guardarMensaje5([{ usuario: usuario, mensaje: mensaje }]);
      ...
      if (file) {
        this.fileDataService.uploadFile(file).subscribe(
          (response) => {
            ...
            StorageHelper.guardarMensaje5([{ usuario: `${this.currentUser.nombre}`, mensaje: `<a href="${fileUrl}" download="${fileName}" target="_blank">${fileName}</a>` }]);
            this.signalrService.sendMessage(message);
          },
          (error) => {
