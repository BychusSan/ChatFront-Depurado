...
import { StorageHelper } from 'src/app/services/localstorage.service';
  ...
  export class Sala4Component implements OnInit, AfterContentInit, OnDestroy {
    ...
    ngOnInit(): void {
      ...
      // this.speak();
      this.mensajes = StorageHelper.obtenerMensajes4() || [];  // TODO: HACER UNA FUNCION PARA GUARDAR LOS MENSAJES EN EL LOCAL STORAGE
        ...
        this.signalrService.messageSubscription.subscribe({
          next: (message: IMessage) => {
            ...
            if (message.room === this.currentUser.room) {
              ...
              StorageHelper.guardarMensaje4([{ usuario: usuario, mensaje: mensaje }]);
      ...
      if (file) {
        this.fileDataService.uploadFile(file).subscribe(
          (response) => {
            ...
            StorageHelper.guardarMensaje4([{ usuario: `${this.currentUser.nombre}`, mensaje: `<a href="${fileUrl}" download="${fileName}" target="_blank">${fileName}</a>` }]);
            this.signalrService.sendMessage(message);
          },
          (error) => {
